...
class OracleGrammar extends Grammar
{
    ...
    protected function compileOrderedRowValueComparison(array $columns, array $values, string $operator): string
    {
        ...
        // Keep UNKNOWN as NULL so an enclosing NOT has the same three-valued
        // semantics as the native row comparison. EXISTS would turn it into false.
        return '(select case when ('.$comparisonSql.') then 1 '
            .'when not ('.$comparisonSql.') then 0 end from (select '.$valuesSql.' from dual) '
            .$this->wrap($valueTable).') = 1';
    }
