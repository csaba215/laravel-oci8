...
class OracleGrammar extends Grammar
{
    ...
    protected function compileAnsiOffset(Builder $query, array $components): string
    {
        ...
        if (version_compare('12c', $query->getConnection()->getConfig('server_version'), '<=') && $query->lock === null) {
            if ($query->limit !== null) {
    ...
    protected function compileUnionLimitOffset(Builder $query, string $sql): string
    {
        ...
        if (version_compare('12c', $query->getConnection()->getConfig('server_version'), '<=')) {
