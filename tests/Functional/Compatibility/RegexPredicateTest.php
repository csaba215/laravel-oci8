...
class RegexPredicateTest extends TestCase
{
    ...
    {
        $ids = $this->matchingIds($this->positiveOperator(), '[[:digit:]]+$');
    ...
    {
        $ids = $this->matchingIds($this->negativeOperator(), '[[:digit:]]+$');
    ...
    public function it_matches_a_regular_expression_case_insensitively(): void
    {
        ...
        $ids = $this->matchingIds($operator, '^alpha-');
    ...
    public function it_matches_a_regular_expression_case_sensitively(): void
    {
        ...
        $ids = $this->matchingIds('~', '^alpha-');

        $this->assertSame([1], $ids);
    }

    private function positiveOperator(): string
    {
        return $this->isPgsql() ? '~' : 'regexp';
    }

    private function negativeOperator(): string
    {
        return $this->isPgsql() ? '!~' : 'not regexp';
    }

    private function matchingIds(string $operator, string $pattern): array
    {
        return DB::table('compatibility_regex_items')
