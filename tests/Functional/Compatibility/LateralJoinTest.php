...
class LateralJoinTest extends TestCase
{
    ...
    public function it_supports_join_lateral_when_the_driver_does()
    {
        ...
        $query = DB::table('users')
            ...
            ->joinLateral($this->latestPostSubquery(), 'latest_post')
            ->orderBy('users.id');
        ...
        $this->skipIfOracleDoesNotSupportLateral();

        $results = $this->normalizeRows($query->get()->all());
    ...
    public function it_supports_left_join_lateral_when_the_driver_does()
    {
        ...
        $query = DB::table('users')
            ->select('users.name', 'latest_post.latest_title')
            ->leftJoinLateral($this->latestPostSubquery(), 'latest_post')
            ->orderBy('users.id');
    ...
    protected function latestPostSubquery()
    {
    ...
    protected function skipIfOracleDoesNotSupportLateral(): void
    {
