...
class ReorderJoinsTest extends TestCase
{
    ...
    {
        $grammar = $this->oracleGrammar();

        $query = DB::table('users')
            ->join(new Expression('(select user_id from "STATS") ds'), 'ds.user_id', '=', 'users.id')
            ->join(new Expression('(select user_id from "STATS") "q_alias"'), 'q_alias.user_id', '=', 'users.id');

        $joins = $query->joins;

        $this->assertSame('ds', $this->invokeExtractJoinAlias($grammar, $joins[0]));
        $this->assertSame('q_alias', $this->invokeExtractJoinAlias($grammar, $joins[1]));
    }
    ...
    public function it_returns_null_for_expression_join_without_alias(): void
    {
        ...
        $query = DB::table('users')
            ->join(new Expression('(select user_id from "STATS")'), 'users.id', '=', 'users.id');

        $join = $query->joins[0];

        $this->assertNull($this->invokeExtractJoinAlias($grammar, $join));
    }
    ...
    public function it_extracts_alias_from_whitespace_heavy_table_and_falls_back_to_table_name(): void
    {
        ...
        $aliasedQuery = DB::table('users')
            ->join('  stats    "S_ALIAS"  ', 'users.id', '=', 'S_ALIAS.user_id');

        $plainQuery = DB::table('users')
            ->join('schema.orders', 'users.id', '=', 'schema.orders.user_id');

        $this->assertSame('S_ALIAS', $this->invokeExtractJoinAlias($grammar, $aliasedQuery->joins[0]));
        $this->assertSame('orders', $this->invokeExtractJoinAlias($grammar, $plainQuery->joins[0]));
    }
    ...
    public function it_keeps_order_when_reorder_is_not_needed_or_aliases_are_unresolved(): void
    {
        ...
        $validSorted = $this->invokeReorderJoins($grammar, $alreadyValid->joins);

        $this->assertSame($alreadyValid->joins[0], $validSorted[0]);
        $this->assertSame($alreadyValid->joins[1], $validSorted[1]);

        $unknownAlias = DB::table('users as u')
            ->join('orders as o', 'o.user_id', '=', 'missing_alias.user_id')
            ->join('stats as s', 's.user_id', '=', 'u.id');

        $unknownSorted = $this->invokeReorderJoins($grammar, $unknownAlias->joins);

        $this->assertSame($unknownAlias->joins[0], $unknownSorted[0]);
        $this->assertSame($unknownAlias->joins[1], $unknownSorted[1]);
    }
    ...
    public function it_reorders_when_a_join_references_a_later_alias_with_stable_topological_order(): void
    {
        ...
        $sorted = $this->invokeReorderJoins($grammar, $query->joins);

        $aliases = array_map(fn ($join) => strtolower((string) $this->invokeExtractJoinAlias($grammar, $join)), $sorted);

        $this->assertSame(['o2', 's', 'o'], $aliases);
    }
    ...
    private function oracleGrammar(): OracleGrammar
    {
        $grammar = DB::query()->getGrammar();
        $this->assertInstanceOf(OracleGrammar::class, $grammar);

        return $grammar;
    }

    private function invokeExtractJoinAlias(OracleGrammar $grammar, $join): ?string
    {
        $method = new ReflectionMethod($grammar, 'extractJoinAlias');
        $method->setAccessible(true);

        return $method->invoke($grammar, $join);
    }

    private function invokeReorderJoins(OracleGrammar $grammar, array $joins): array
    {
        $method = new ReflectionMethod($grammar, 'reorderJoins');
        $method->setAccessible(true);

        return $method->invoke($grammar, $joins);
    }
}
