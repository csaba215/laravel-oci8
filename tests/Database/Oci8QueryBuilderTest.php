...
class Oci8QueryBuilderTest extends TestCase
{
    ...
    public function test_union_limit_with_aliases_and_schema_qualified_tables()
    {
        $builder = $this->getBuilder();
        $builder->select('u.id as user_id', 'u.name')
            ->from('reporting.users as u')
            ->where('u.active', 1);
        $builder->union(
            $this->getBuilder()
                ->select('a.id as user_id', 'a.name')
                ->from('archive.users as a')
                ->where('a.active', 0)
        );
        $builder->orderBy('user_id')->skip(1)->take(2);

        if ($this->getConnection()->isVersionAboveOrEqual('12c')) {
            $this->assertSame('(select "U"."ID" as "USER_ID", "U"."NAME" from "REPORTING"."USERS" "U" where "U"."ACTIVE" = ?) union (select "A"."ID" as "USER_ID", "A"."NAME" from "ARCHIVE"."USERS" "A" where "A"."ACTIVE" = ?) order by "USER_ID" asc offset 1 rows fetch next 2 rows only', $builder->toSql());
        } else {
            $this->assertSame('select t2."USER_ID", t2."NAME" from ( select rownum AS "rn", t1.* from ((select "U"."ID" as "USER_ID", "U"."NAME" from "REPORTING"."USERS" "U" where "U"."ACTIVE" = ?) union (select "A"."ID" as "USER_ID", "A"."NAME" from "ARCHIVE"."USERS" "A" where "A"."ACTIVE" = ?) order by "USER_ID" asc) t1 ) t2 where t2."rn" between 2 and 3', $builder->toSql());
        }

        $this->assertSame([1, 0], $builder->getBindings());
    }

    public function test_union_limit_with_joins_that_duplicate_target_rows_keeps_binding_order()
    {
        $builder = $this->getBuilder();
        $builder->select('users.id as id', 'users.name as name')
            ->from('users')
        ...
        $builder->unionAll(
            $this->getBuilder()
                ...
                ->join('user_events', function ($join) {
                    $join->on('users.id', '=', 'user_events.user_id')
                        ->where('user_events.kind', '=', 'purchase');
                })
                ->where('users.active', 0)
        );
        $builder->orderBy('id')->skip(2)->take(4);

        if ($this->getConnection()->isVersionAboveOrEqual('12c')) {
            $this->assertSame('(select "USERS"."ID" as "ID", "USERS"."NAME" as "NAME" from "USERS" inner join "USER_EVENTS" on "USERS"."ID" = "USER_EVENTS"."USER_ID" and "USER_EVENTS"."KIND" = ? where "USERS"."ACTIVE" = ?) union all (select "USERS"."ID" as "ID", "USERS"."NAME" as "NAME" from "USERS" inner join "USER_EVENTS" on "USERS"."ID" = "USER_EVENTS"."USER_ID" and "USER_EVENTS"."KIND" = ? where "USERS"."ACTIVE" = ?) order by "ID" asc offset 2 rows fetch next 4 rows only', $builder->toSql());
        } else {
            $this->assertSame('select t2."ID", t2."NAME" from ( select rownum AS "rn", t1.* from ((select "USERS"."ID" as "ID", "USERS"."NAME" as "NAME" from "USERS" inner join "USER_EVENTS" on "USERS"."ID" = "USER_EVENTS"."USER_ID" and "USER_EVENTS"."KIND" = ? where "USERS"."ACTIVE" = ?) union all (select "USERS"."ID" as "ID", "USERS"."NAME" as "NAME" from "USERS" inner join "USER_EVENTS" on "USERS"."ID" = "USER_EVENTS"."USER_ID" and "USER_EVENTS"."KIND" = ? where "USERS"."ACTIVE" = ?) order by "ID" asc) t1 ) t2 where t2."rn" between 3 and 6', $builder->toSql());
        }

        $this->assertSame(['login', 1, 'purchase', 0], $builder->getBindings());
    }
