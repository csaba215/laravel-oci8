...
class Oci8QueryBuilderTest extends TestCase
{
    ...
    public function test_union_limits_and_offsets()
    {
        ...
        if (version_compare('12c', $serverVersion, '<=')) {
    ...
    public function test_sub_select_where_ins()
    {
        ...
        if (version_compare('12c', $this->getConnection()->getConfig('server_version'), '<=')) {
            $this->assertEquals(
    ...
    public function test_find_returns_first_result_by_id()
    {
        ...
        $builder->getConnection()
            ...
            ->once()
            ->with(version_compare('12c', $this->getConnection()->getConfig('server_version'), '<=') ? 'select /*+ FIRST_ROWS(1) */ * from "USERS" where "ID" = ? offset 0 rows fetch next 1 rows only' : 'select * from (select * from "USERS" where "ID" = ?) where rownum = 1', [1], true)
            ->andReturn([['foo' => 'bar']]);
    ...
    public function test_first_method_returns_first_result()
    {
        ...
        $builder->getConnection()->shouldReceive('select')
            ->once()
            ->with(version_compare('12c', $this->getConnection()->getConfig('server_version'), '<=') ? 'select /*+ FIRST_ROWS(1) */ * from "USERS" where "ID" = ? offset 0 rows fetch next 1 rows only' : 'select * from (select * from "USERS" where "ID" = ?) where rownum = 1',
                [1], true)
            ->andReturn([['foo' => 'bar']]);
        $builder->getProcessor()
            ->shouldReceive('processSelect')
    ...
    {
        $builder = $this->getBuilder();
        $builder->getConnection()->shouldReceive('select')->once()->with(version_compare('12c', $this->getConnection()->getConfig('server_version'), '<=') ? 'select /*+ FIRST_ROWS(1) */ "FOO" from "USERS" where "ID" = ? offset 0 rows fetch next 1 rows only' : 'select * from (select "FOO" from "USERS" where "ID" = ?) where rownum = 1', [1], true)->andReturn([['foo' => 'bar']]);
