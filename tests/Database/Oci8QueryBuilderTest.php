...
class Oci8QueryBuilderTest extends TestCase
{
    ...
    public function test_union_with_zero_offset_and_no_limit()
    {
        $builder = $this->getBuilder();
        $builder->select('*')->from('users');
        $builder->union($this->getBuilder()->select('*')->from('dogs'));
    ...
    public function test_union_limit_uses_alias_for_outer_projection()
    {
        $builder = $this->getBuilder();
        $builder->selectRaw('id as user_id')->from('users');
        $builder->union($this->getBuilder()->selectRaw('id as user_id')->from('dogs'));
        $builder->take(10);

        if ($this->getConnection()->isVersionAboveOrEqual('12c')) {
            $this->assertSame('(select id as user_id from "USERS") union (select id as user_id from "DOGS") offset 0 rows fetch next 10 rows only', $builder->toSql());
        } else {
            $this->assertSame('select t2."USER_ID" from ( select rownum AS "rn", t1.* from ((select id as user_id from "USERS") union (select id as user_id from "DOGS")) t1 ) t2 where t2."rn" between 1 and 10', $builder->toSql());
        }
    }

    public function test_union_limit_falls_back_to_all_columns_when_selecting_rn()
    {
        $builder = $this->getBuilder();
        $builder->selectRaw('id as rn')->from('users');
        $builder->union($this->getBuilder()->selectRaw('id as rn')->from('dogs'));
        $builder->take(10);

        if ($this->getConnection()->isVersionAboveOrEqual('12c')) {
            $this->assertSame('(select id as rn from "USERS") union (select id as rn from "DOGS") offset 0 rows fetch next 10 rows only', $builder->toSql());
        } else {
            $this->assertSame('select t2.* from ( select rownum AS "rn", t1.* from ((select id as rn from "USERS") union (select id as rn from "DOGS")) t1 ) t2 where t2."rn" between 1 and 10', $builder->toSql());
        }
    }

    public function test_union_limit_falls_back_to_all_columns_when_selecting_wildcard_column()
    {
        $builder = $this->getBuilder();
        $builder->select('users.*')->from('users');
        $builder->union($this->getBuilder()->select('dogs.*')->from('dogs'));
        $builder->take(10);

        if ($this->getConnection()->isVersionAboveOrEqual('12c')) {
            $this->assertSame('(select "USERS".* from "USERS") union (select "DOGS".* from "DOGS") offset 0 rows fetch next 10 rows only', $builder->toSql());
        } else {
            $this->assertSame('select t2.* from ( select rownum AS "rn", t1.* from ((select "USERS".* from "USERS") union (select "DOGS".* from "DOGS")) t1 ) t2 where t2."rn" between 1 and 10', $builder->toSql());
        }
    }
    ...
    public function test_union_limit_falls_back_to_all_columns_when_selecting_expression_without_alias()
    {
        ...
        $builder->union($this->getBuilder()->selectRaw('lower(name)')->from('dogs'));
        $builder->take(10);
    ...
    {
        $condition = '"LAST_UPDATE" < "LARAVEL_ROW_VALUES"."LARAVEL_ROW_VALUE_0" '
            .'or ("LAST_UPDATE" = "LARAVEL_ROW_VALUES"."LARAVEL_ROW_VALUE_0" '
            .'and "ORDER_NUMBER" < "LARAVEL_ROW_VALUES"."LARAVEL_ROW_VALUE_1")';
        $comparison = '(select case when ('.$condition.') then 1 when not ('.$condition.') then 0 end from '
            .'(select ? as "LARAVEL_ROW_VALUE_0", ? as "LARAVEL_ROW_VALUE_1" from dual) '
            .'"LARAVEL_ROW_VALUES") = 1';
        ...
        $condition = '"LAST_UPDATE" < "LARAVEL_ROW_VALUES"."LARAVEL_ROW_VALUE_0" '
            .'or ("LAST_UPDATE" = "LARAVEL_ROW_VALUES"."LARAVEL_ROW_VALUE_0" and "ORDER_NUMBER" < 2)';
        $this->assertSame(
            'select * from "ORDERS" where (select case when ('.$condition.') then 1 when not ('
            .$condition.') then 0 end from (select ? as "LARAVEL_ROW_VALUE_0" from dual) '
            .'"LARAVEL_ROW_VALUES") = 1',
            $builder->toSql()
        );
        $this->assertEquals([1], $builder->getBindings());
    ...
    public function test_negated_ordered_row_values_preserve_null_semantics()
    {
        $condition = '"SCORE" < "LARAVEL_ROW_VALUES"."LARAVEL_ROW_VALUE_0"';
        $comparison = '(select case when ('.$condition.') then 1 when not ('.$condition.') then 0 end from '
            .'(select ? as "LARAVEL_ROW_VALUE_0" from dual) "LARAVEL_ROW_VALUES") = 1';

        $builder = $this->getBuilder();
        $builder->select('*')->from('orders')
            ->whereNot(fn ($query) => $query->whereRowValues(['score'], '<', [10]));

        $this->assertSame('select * from "ORDERS" where not ('.$comparison.')', $builder->toSql());
    ...
    public function test_where_row_values_with_non_sequential_keys()
    {
        $builder = $this->getBuilder();
        $builder->select('*')->from('orders')->whereRowValues(
            [2 => 'last_update', 4 => 'order_number'],
            '=',
            [1 => 1, 3 => 2]
        );
        $this->assertSame('select * from "ORDERS" where ("LAST_UPDATE" = ? and "ORDER_NUMBER" = ?)', $builder->toSql());
        ...
        $this->assertSame(
            'select * from "ORDERS" where (select case when ('.$condition.') then 1 when not ('
            .$condition.') then 0 end from (select ? as "LARAVEL_ROW_VALUE_0", '
            .'? as "LARAVEL_ROW_VALUE_1" from dual) "LARAVEL_ROW_VALUES") = 1',
            $builder->toSql()
        );
        $this->assertEquals([1, 2], $builder->getBindings());
    }

    public function test_ordered_row_values_use_a_non_conflicting_internal_alias()
    {
        $builder = $this->getBuilder();
        $builder->select('*')->from('orders as laravel_row_values')->whereRowValues(
            ['laravel_row_values.last_update', 'laravel_row_values.order_number'],
            '<',
            [1, 2]
        );
        $condition = '"LARAVEL_ROW_VALUES"."LAST_UPDATE" '
            .'< "LARAVEL_ROW_VALUES_1"."LARAVEL_ROW_VALUE_0" or '
            .'("LARAVEL_ROW_VALUES"."LAST_UPDATE" = "LARAVEL_ROW_VALUES_1"."LARAVEL_ROW_VALUE_0" '
            .'and "LARAVEL_ROW_VALUES"."ORDER_NUMBER" < "LARAVEL_ROW_VALUES_1"."LARAVEL_ROW_VALUE_1")';

        $this->assertSame(
            'select * from "ORDERS" "LARAVEL_ROW_VALUES" where (select case when ('.$condition
            .') then 1 when not ('.$condition.') then 0 end from (select ? as "LARAVEL_ROW_VALUE_0", '
            .'? as "LARAVEL_ROW_VALUE_1" from dual) "LARAVEL_ROW_VALUES_1") = 1',
            $builder->toSql()
        );
        $this->assertEquals([1, 2], $builder->getBindings());
    }

    public function test_ordered_row_values_do_not_shadow_unqualified_outer_columns()
    {
        $builder = $this->getBuilder();
        $builder->select('*')->from('orders')->whereRowValues(
            ['laravel_row_value_0', 'order_number'],
            '<',
            [1, 2]
        );
        $condition = '"LARAVEL_ROW_VALUE_0" '
            .'< "LARAVEL_ROW_VALUES"."LARAVEL_ROW_VALUE_0_1" or '
            .'("LARAVEL_ROW_VALUE_0" = "LARAVEL_ROW_VALUES"."LARAVEL_ROW_VALUE_0_1" '
            .'and "ORDER_NUMBER" < "LARAVEL_ROW_VALUES"."LARAVEL_ROW_VALUE_1")';

        $this->assertSame(
            'select * from "ORDERS" where (select case when ('.$condition.') then 1 when not ('
            .$condition.') then 0 end from (select ? as "LARAVEL_ROW_VALUE_0_1", '
            .'? as "LARAVEL_ROW_VALUE_1" from dual) "LARAVEL_ROW_VALUES") = 1',
            $builder->toSql()
        );
        $this->assertEquals([1, 2], $builder->getBindings());
    ...
    public function test_ordered_row_values_keep_outer_referencing_expressions_visible()
    {
        $builder = $this->getBuilder();
        $builder->select('*')->from('events as records')->whereRowValues(
            ['records.start_at'],
            '<',
            [new Raw('records.end_at')]
        );

        $this->assertSame(
            'select * from "EVENTS" "RECORDS" where ("RECORDS"."START_AT" < records.end_at)',
            $builder->toSql()
        );
        $this->assertSame([], $builder->getBindings());

        $builder = $this->getBuilder();
        $builder->select('*')->from('events as laravel_row_values')->whereRowValues(
            ['start_at', 'id'],
            '<',
            [new Raw('laravel_row_values.end_at'), 10]
        );
        $condition = '"START_AT" < laravel_row_values.end_at or '
            .'("START_AT" = laravel_row_values.end_at '
            .'and "ID" < "LARAVEL_ROW_VALUES_1"."LARAVEL_ROW_VALUE_1")';

        $this->assertSame(
            'select * from "EVENTS" "LARAVEL_ROW_VALUES" where (select case when ('.$condition
            .') then 1 when not ('.$condition.') then 0 end from '
            .'(select ? as "LARAVEL_ROW_VALUE_1" from dual) "LARAVEL_ROW_VALUES_1") = 1',
            $builder->toSql()
        );
        $this->assertEquals([10], $builder->getBindings());
    }
